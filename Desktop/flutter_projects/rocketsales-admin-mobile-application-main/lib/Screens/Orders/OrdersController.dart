...
// ✅ Correct grand total calculation
  ...
  Future<void> getOrders() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/getorder?&limit=10$dateTimeFilter&search=$searchString&status=$selectedTag&salesmanId=$salesmanId');
  ...
  void getMoreOrderCards() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/getorder?page=$page&limit=10$dateTimeFilter&search=$searchString&salesmanId=$salesmanId&status=$selectedTag');
      final response = await http.get(
        url,
        headers: {
          'Content-Type': 'application/json',
