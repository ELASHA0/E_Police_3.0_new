...
class LeavesController extends GetxController {
  ...
  Future<void> getLeaves() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/leaverequest?limit=10${dateTimeFilter.value == "" ? "&filter=thisMonth" : dateTimeFilter.value}&search=$searchString&status=$selectedTag');
  ...
  void getMoreLeavesCards() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/leaverequest?page=$page&limit=10$dateTimeFilter&search=$searchString&status=$selectedTag');
      final response = await http.get(
        url,
        headers: {
          'Content-Type': 'application/json',
