...
class TaskController extends GetxController {
  ...
  void getTasks() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/get-task?&limit=20$dateTimeFilter&search=$searchString&status=$selectedTag&salesmanId=$salesmanId');
      final response = await http.get(
        url,
        headers: {
          'Content-Type': 'application/json',
  ...
  void getMoreTaskCards() async {
    ...
    try {
      ...
      final url = Uri.parse(
          '${dotenv.env['BASE_URL']}/api/api/get-task?page=$page&limit=10$dateTimeFilter&search=$searchString&status=$selectedTag&salesmanId=$salesmanId');
      final response = await http.get(
        url,
        headers: {
          'Content-Type': 'application/json',
